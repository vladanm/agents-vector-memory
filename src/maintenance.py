...
class MaintenanceOperations:
    ...
    def verify_embedding_coverage(self) -> dict[str, Any]:
        """
        Verify all chunks have embeddings.

        This method checks what percentage of chunks in the database have
        embeddings in the vec_chunk_search table. It's useful for:
        - Verifying backfill operations completed successfully
        - Monitoring embedding coverage during operations
        - Detecting chunks that may have been created without embeddings
        ...
        try:
            ...
            with_embeddings = conn.execute("""
                ...
                INNER JOIN vec_chunk_search v ON c.id = v.chunk_id
            """).fetchone()[0]

            missing = total - with_embeddings
            coverage = (with_embeddings / total * 100) if total > 0 else 100.0
            ...
            return {
                ...
                "message": f"Embedding coverage: {coverage:.1f}% ({with_embeddings}/{total})"
            }
